...
5장. '데이터베이스' 조작이 편해지는 'ORM'
    1. 데이터베이스(database)란?
        ...
        DBMS(DataBase Management System) : 기본적으로 데이터 베이스는 많은 사람이 공유할 수 있어야 하고,
            동시 접근이 가능해야 한다 등 많은 요구 사항이 존재함.
            ...
            H2, MySQL
                ...
                H2 - 자바로 작성되어 있는 RDBMS : 스프링 부트가 지원하는 인메모리 관계형 데이터베이스
                    ...
                    실제 서비스에서는 사용하지 않습니다.
                MySQL - 실제 서비스로 올릴 때 사용함 RDBMS -> 추후 수업 예정

            필수 용어
                1) 테이블 : DB에서 데이터를 구성하기 위한 가장 기분적인 단위 행과 열로 구성되며
                ...
                3) 열(column) : 테이블의 구성 요소 중 하나이며, 행에 저장되는 유형의 데이터.
                    ...
                    이메일 열에 수수자가 들어가거나, 나이 열에 문자열이 들어갈 수 없기 때문에 데이터에 대한 무결성을 보장
                4) 기본키(Primary Key) : 행을 구분할 수 있는 식별자. 이 값은 테이블에서 유일해야 하며 '중복값을 가질 수 없음'
                    ...
                    또한 기본키의 값은 수정되어서는 안 되며 유효한 값이여야 함. 즉, Null이 될 수 없음.
    ...
    2. ORM이란?
        Object-Relational Mapping이라고 하며 자바의 객체와 데이터베이스를 연결하는 프로그래밍 기법
            예를 들어 데이터베이스에 age, name 컬럼에 20, 홍길동이라는 값이 있다고 저장했을 때, 이것을 자바에서 사용하려면
            ...
            즉, 객체의 데이터베이스를 연결해 자바 언어로만 데이터 베이스르 다룰 수 있도록 하는 도구를 ORM이라고 함.

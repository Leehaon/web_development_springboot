...
/*
    1. 사전 지식 : 타임리프(Thymeleaf)
        타임리프는 템플릿 엔진입니다. 다만 템플렛 엔즌은 HTML과 템플릿 엔진을 위한 문법을 섞어서 사용.
            즉 이제는 HTML 지식이 약간 필요합니다.

            -> 저희는 다음 과정에서 HTML을 배우게 됴ㅚ므로 아준 간단한 화면 구성을 위해서 코드를 작성할 예정
        ...
        * 템플릿 엔진 : 스프링 서버에서 데이터를 받아 우리가 보는 웹 페이지, 즉 HTML 상에서
        ...
        템플릿 엔진 개념 잡기
            ...
            h1 태그에서는 ${이름}이 텍스트 어트리뷰트(속성)로 활당돼있습니다.
            p 태그도 비슷한 상황입니다.
            이상이 템플릿 문법의 예시인데,
            ...
            ↓ 서버에서 보내준 데이터의 예
            {
             ...
             값이 달라지면 그때그때 화면에 반영하니 동작인 웹 페이지를 만들 수 있게 되는 것입니다.

             템플릿 엔진은 각각 문법이 조금씩 다른 편입니다. 그래서 템플릿 엔진마다 문법을 새로 배워야 하는
             단점은 존재합니다만 구조는 그래도 비슷합니다.
                대표적인 템플릿 엔진 예시 : JSP, 타입리프, 프리마커 등

                하지만 스프링 타입리프를 권장하고 있으므로 우리는 타임리프 사용 예정

            타임리프 표현식과 문법
                타임리프의 문법은 직관적인 편인데, 자주 사용하느 표현식과 문법은 예시로만 보여드리고,
                구체적인 사용 방법은 실습으로 알아볼겁니다.

                이하에서 소개하느 표현식들은 전달받은 데이터를 사용자들이 볼 수 있게
                뷰로 만들기 위해 시용되는 표현식 예시입니다.
            ...
            *{...} - 선택한 변수의 표현식.th:object에서 선택한 객체에 접근
            ...
            th:if - 조건이 true인 때만 표시 - th:id"${person.age}>=20"

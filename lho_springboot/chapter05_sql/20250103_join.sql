...
-- 3. subquery
-- 
-- 1 - 1. SQL에서 JOIN은 여러 테이블에서 데이터를 가져와 결합하느 기능을 합니다.
-- RDBS에서는 데이터의 중복을 피하고 쉽게 관리하기 위해 데이터를 여러 곳에 나누어 보관합니다.
-- 	ex) 어떤 주문을 담당한 직원을 확인하고 싶은데, 주문 아이디 orders 테이블에 있고,
-- 		직원 이름을 staff 테이블에 있으면 어떻게 쿼리를 입력해야 하는지 고민해보도록 합니다.
-- 		
-- 	이상에서 이루어지는 데이터 분산을 '데이터의 정규화(data normalization)'이라고 하며
-- 	데이터베이스에서 중복을 최소화하고 데이터의 일관성을 유지하기 위함.
-- 	
-- 	1) JOIN
-- 		지금까지는 테입르 하나의 데이터를 필터링 등 다루는 연습을 했습니다.
-- 		이제는 둘 이상의 테이블을 함께 다루면서 해당 키워드를 통해 합쳐서 보는 연습을 합니다.
-- 		
-- 		실습 데이터를 기준으로,
-- 		ex) 국갑려로 주문 건수르 알아보고 싶다면 -> users와 orders가 필요함.

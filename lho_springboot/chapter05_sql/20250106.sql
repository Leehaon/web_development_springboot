...
-- FROM user s 로 작성했을 때 이후에는 u만 썼을 경우 users 테이블을 의미하게 됨.
-- 그래서 이후에 FROM 절에서 다수의 테이블명을 가입하게 될 경우에 별칭을 통해서 정리하여
-- SQL문을 효율적응로 사용할 수 있게 됨.
...
-- 키 값은 테이블에 반드시 1 개 이상 존재하도록 설계되어 있고(여러분이 설계해야 하고),
-- 테이블에서 개별 행을 유일하게 구분 짓습니다. 따라서 키 갑은 컬럼 내에서 중복되지 않으며
...
-- 세번 주문했다면 회원 정보(users)와 주문 정보(orders)를 결합한 결과에는 u.id = 7인
...
-- FK는 다른 테이블의 고유한 키 값이 PK를 참조한다.(orders에서 o.id가 FK에 해당,
...
-- 2. LEFT JOIN
-- useras와 orders를 하나로 결합해 출력합니다(단, 주문 정보가 없는 회원의 정보도 모두 출력)
...
-- LEFT JOIN은 INNER JOIN의 결과값인 교집합 뿐만 아니라 JOIN 명령문의 왼쪽에 해당한 위치에 테이블의
-- 모든 결과값을 가져옵니다.

-- 이상의 예시에서는 users 테이블이 외쪽에 있으므로, users 테이블의 모든 결과값을 가져오는데,
-- orders 테이블에 대응하는 값이 없는 경우 null 값으로 출력됩니다.

-- 즉, 두 테이블의 교집합과 교집합에 속하지 않는 왼쪽 차집합을 불러옵니다.
-- 왼쪽 테입르의 값을 전부 불러오기 때문에 LEFT JOIN / LEFT OUTER JOIN이라고 합니다/

-- LEFT JOIN과 INNER JOIN은 함께 실무에서 자주 쓰입니다. 데이터를 결합하는 경우 대분분 
...
-- 두 테이블에 동일한 컬럼이 있을 때(대부분 경우 PK 역할을 하는 id들은 다 있으니까요),
...
-- FROM 내에서는 JOIN을 중첩해서 횟숫 제한 없이 사용 가능합니다.
...
-- 따라서 orders 테이블의 user_id 컬럼은 모두 users 테이블 id 컬럼에 있는 값에 해당합니다.
-- 결합으로 봤을 때 orders는 users 테이블에 종속돼있으므로 null 값이 출려될 수 없습니다.
...
-- CROOS JOIN - 두 테이블 간의 집합을 조합해 만들 수 있는 모든 경우의 수를 생성하느 방식으로,
...
-- 실직적으로는 NULL 값만 출력되는 경우의 수가 많아 쓸모는 없기 때문입니다.
SELECT * FROM staff;
SELECT * FROM users;
-- 연습 문제
...
-- 4. orders와 orderdetail, products를 참고하여 회원 아이디별 주문 금액의 총합을 정상 가격과 

...
/*
    학습 목표 : 로그인 / 회원 가입 기능을 구현
        ...
        -> defaultSuccessUrl로 설정한 / articles로 리타이렉트 하거나 csrf를 disable 하거나 하는 등의 작업

        UserDetailsService에서는 loadUserByUsername() 메서드를 실행하여 이메일로 유지를 찾고 반환.
        ...
        로그아웃
        /logout 요청이 들어오면
        -> UserApiController클래스에서 로그아웃 로직을 실행할 얘정.
        그 로그아웃 로직 : SecurityContextLogoutHander에서 제공하는 logout() 메서드를 실행할 얘정

        1. 스프링 시큐리티(Spring Security)
            1) 인증 vs 인가
                (1) 인증(Authentication) : 사용자의 신원을 인증하는 과정
            ...
            3) 필터 기반으로 동작하는 스프링 시쿠리티
                스프링 시큐리티의 플 로그인 과정을 설정하는 건(코드로 작성하는 건) 그렇게 어렵지 않을건데,
    ...
    2. 회원 도메인 만들기
        스프링 시큐리티를 활용한 인증, 인가 가능 구현 예정.
        회원 정보를 저장할 테이블을 만들고,
        테이블과 연결할 도메인을 만들고,
        이 테이블과 연결할 회원 엔티티를 만들 겁니다.
        회원 엔티티와 연결되어 데이터를 조회하게 해줄 리프지토리가 추가되구요, 마지막으로
